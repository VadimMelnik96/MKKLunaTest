...
class SQLAlchemyRepository(IRepository):
    ...
    async def count(self, filters: BaseFilter | None = None) -> int:
        ...
        if filters:
            stmt = self._build_filter_query(stmt=stmt, filters=filters)

            # count всегда без пагинации
            if hasattr(filters, "pagination") and filters.pagination:
                stmt = stmt.limit(None).offset(None)
    ...
    async def list_and_count(
        ...
        if filters:
            count_stmt = self._build_filter_query(stmt=count_stmt, filters=filters)

            if hasattr(filters, "pagination") and filters.pagination:
                count_stmt = count_stmt.limit(None).offset(None)

        total = (await self._execute(count_stmt)).scalar_one()

        list_stmt = self._build_filter_query(
            stmt=select(self.model),
            filters=filters,
        )

        res = await self._execute(list_stmt)
        items = self.to_dto(res.scalars(), response_dto)
